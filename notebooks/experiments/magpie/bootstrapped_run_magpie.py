...
# RESULTS
name_prefix = f"BOOTSTRAPPED_MAGPIE_batchsize{batch_size}_lyapunovpenalty{lyapunov_penalty}_latents{use_latents}_swa{swa}_seed{SEED}"
SAVE_PLOT = True
CHECKPOINTING = False
...
base_dir = './data/'
split_dir = 'GSEA_HYPOXIA_FIXED'
data_dir = os.path.join(base_dir,split_dir)

samples_train = np.load(os.path.join(data_dir,'X_train_COUNTS.npy'))
samples_valid = np.load(os.path.join(data_dir,'X_val_COUNTS.npy'))
samples_test = np.load(os.path.join(data_dir,'X_holdout_COUNTS.npy'))

regimes_train = np.load(os.path.join(data_dir,'y_train.npy'))
regimes_valid = np.load(os.path.join(data_dir,'y_val.npy'))
regimes_test = np.load(os.path.join(data_dir,'y_holdout.npy'))

train_gene_ko = np.unique( np.concatenate( (regimes_train, regimes_valid), axis = 0) )
test_gene_ko = np.unique( regimes_test )
...
print('Overlap:')
print( set(train_gene_ko).intersection(set(test_gene_ko)) )

gt_interv = torch.tensor(np.load(os.path.join(data_dir,'gt_interv.npy')), dtype = torch.float)

with open(os.path.join(data_dir,'var_names.pkl'), 'rb') as f:
    labels = pickle.load(f)

# Concatenate data

samples = torch.tensor( np.concatenate( (samples_train, samples_valid, samples_test), axis = 0 ), dtype = torch.float )
regime = torch.tensor( np.concatenate( (regimes_train, regimes_valid, regimes_test), axis = 0 ) )
