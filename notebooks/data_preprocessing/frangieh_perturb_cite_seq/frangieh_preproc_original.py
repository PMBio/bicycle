...
DATA_PATH = Path("/scratch/ueltzhoe")
'''
#
# Notebook 1
...
chosen_genes = pd.read_csv("220930_PerturbCITE_regulators.csv", index_col=0)
# ADDED BY MARTIN
...
adata_gf[adata_gf.obs.condition == "Control"].copy().write_h5ad(
    DATA_PATH / "SCP1064/ready/control/gene_filtered_adata.h5ad"
)
adata_gf[adata_gf.obs.condition == "Co-culture"].copy().write_h5ad(
    DATA_PATH / "SCP1064/ready/cocult/gene_filtered_adata.h5ad"
)
adata_gf[adata_gf.obs.condition == "IFNγ"].copy().write_h5ad(
    DATA_PATH / "SCP1064/ready/ifn/gene_filtered_adata.h5ad"
)
...
adata = sc.read_h5ad(DATA_PATH / "SCP1064/ready/control/gene_filtered_adata.h5ad")

counts = False

def generate_nodags_data(adata_path, counts = False):
    adata = sc.read_h5ad(adata_path)
    if counts == False:
        data = sp.csr_matrix.toarray(adata.X)
    else:
        data = sp.csr_matrix.toarray(adata.layers["counts"])
...
control_datasets, control_interventions = generate_nodags_data(
    DATA_PATH / "SCP1064/ready/control/gene_filtered_adata.h5ad", counts
)
cocult_datasets, cocult_interventions = generate_nodags_data(
    DATA_PATH / "SCP1064/ready/cocult/gene_filtered_adata.h5ad", counts
)
ifn_datasets, ifn_interventions = generate_nodags_data(
    DATA_PATH / "SCP1064/ready/ifn/gene_filtered_adata.h5ad", counts
)
...
for condition in ['control','cocult','ifn']:
    for split in ['training_data','validation_data','test_data']:
        temp_path = DATA_PATH / "nodags_data" / condition / split
...
for i, dataset in enumerate(control_training_data):
    np.save(DATA_PATH / "nodags_data/control/training_data/dataset_{}.npy".format(i), dataset)
np.save(DATA_PATH / "nodags_data/control/training_data/intervention_sets.npy", control_training_interventions)

for i, dataset in enumerate(control_validation_data):
    np.save(DATA_PATH / "nodags_data/control/validation_data/dataset_{}.npy".format(i), dataset)
np.save(
    DATA_PATH / "nodags_data/control/validation_data/intervention_sets.npy", control_validation_interventions
)

# saving cocult data
for i, dataset in enumerate(cocult_training_data):
    np.save(DATA_PATH / "nodags_data/cocult/training_data/dataset_{}.npy".format(i), dataset)
np.save(DATA_PATH / "nodags_data/cocult/training_data/intervention_sets.npy", cocult_training_interventions)

for i, dataset in enumerate(cocult_validation_data):
    np.save(DATA_PATH / "nodags_data/cocult/validation_data/dataset_{}.npy".format(i), dataset)
np.save(
    DATA_PATH / "nodags_data/cocult/validation_data/intervention_sets.npy", cocult_validation_interventions
)

# saving ifn data
for i, dataset in enumerate(ifn_training_data):
    np.save(DATA_PATH / "nodags_data/ifn/training_data/dataset_{}.npy".format(i), dataset)
np.save(DATA_PATH / "nodags_data/ifn/training_data/intervention_sets.npy", ifn_training_interventions)

for i, dataset in enumerate(ifn_validation_data):
    np.save(DATA_PATH / "nodags_data/ifn/validation_data/dataset_{}.npy".format(i), dataset)
np.save(DATA_PATH / "nodags_data/ifn/validation_data/intervention_sets.npy", ifn_validation_interventions)

# saving control data
for i, dataset in enumerate(control_datasets):
    np.save(DATA_PATH / "nodags_data/control/dataset_{}.npy".format(i), dataset)
np.save(DATA_PATH / "nodags_data/control/intervention_sets.npy", control_interventions)

# saving cocult data
for i, dataset in enumerate(cocult_datasets):
    np.save(DATA_PATH / "nodags_data/cocult/dataset_{}.npy".format(i), dataset)
np.save(DATA_PATH / "nodags_data/cocult/intervention_sets.npy", cocult_interventions)

# saving ifn data
for i, dataset in enumerate(ifn_datasets):
    np.save(DATA_PATH / "nodags_data/ifn/dataset_{}.npy".format(i), dataset)
np.save(DATA_PATH / "nodags_data/ifn/intervention_sets.npy", ifn_interventions)

np.save(DATA_PATH / "nodags_data/control/training_data/weights.npy", np.eye(61))
np.save(DATA_PATH / "nodags_data/cocult/training_data/weights.npy", np.eye(61))
np.save(DATA_PATH / "nodags_data/ifn/training_data/weights.npy", np.eye(61))

np.save(DATA_PATH / "nodags_data/control/validation_data/weights.npy", np.eye(61))
np.save(DATA_PATH / "nodags_data/cocult/validation_data/weights.npy", np.eye(61))
np.save(DATA_PATH / "nodags_data/ifn/validation_data/weights.npy", np.eye(61))
...
for name, train_samples_, valid_samples_, train_interv_, valid_interv_ in [
    ...
    np.save(DATA_PATH / f"nodags_data/{name}/training_data/obs_means.npy", obs_data_mu)
    ...
    torch.save(train_loader, DATA_PATH / f"nodags_data/{name}/training_data/train_loader.pth")
    torch.save(validation_loader, DATA_PATH / f"nodags_data/{name}/validation_data/validation_loader.pth")
    torch.save(test_loader, DATA_PATH / f"nodags_data/{name}/test_data/test_loader.pth")
    # Save labels
    np.save(DATA_PATH / f"nodags_data/{name}/labels.npy", adata.var.index.values)
